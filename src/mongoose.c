...
struct ns_connection *ns_add_sock(struct ns_mgr *, sock_t, ns_callback_t,
		void *);
struct ns_connection *ns_bind(struct ns_mgr *, const char *, ns_callback_t,
		void *);
struct ns_connection *ns_connect(struct ns_mgr *, const char *, ns_callback_t,
		void *);
...
static size_t ns_out(struct ns_connection *nc, const void *buf, size_t len)
{
	if (nc->flags & NSF_UDP)
	{
		long n = sendto(nc->sock, (const char *) buf, len, 0, &nc->sa.sa,
				sizeof(nc->sa.sin));
		DBG(
				("%p %d send %ld (%d %s)", nc, nc->sock, n, errno, strerror(errno)));
		return n < 0 ? 0 : n;
	}
	else
	{
		...
		*buf = NULL;
		while (len < 0)
		{
			if (*buf)
				NS_FREE(*buf);
			size *= 2;
			if ((*buf = (char *) NS_MALLOC(size)) == NULL)
				break;
			va_copy(ap_copy, ap);
			len = vsnprintf(*buf, size, fmt, ap_copy);
			va_end(ap_copy);
...
static void hexdump(struct ns_connection *nc, const char *path, int num_bytes,
		int ev)
{
	...
	if ((fp = fopen(path, "a")) != NULL)
	{
		...
		fprintf(fp, "%lu %p %s %s %s %d\n", (unsigned long) time(NULL),
				...
				ev == NS_ACCEPT ? "<A" : ev == NS_CONNECT ? "C>" : "XX", dst,
				num_bytes);
		if (num_bytes > 0 && (buf = (char *) NS_MALLOC(buf_size)) != NULL)
		{
			ns_hexdump(
					io->buf + (ev == NS_SEND ? 0 : io->len)
							- (ev == NS_SEND ? 0 : num_bytes), num_bytes, buf,
					buf_size);
			fprintf(fp, "%s", buf);
			NS_FREE(buf);
		}
		...
		int *proto, int *use_ssl, char *cert, char *ca)
{
	...
	*use_ssl = 0;
	...
	if (*use_ssl
			&& (sscanf(str + len, ":%99[^:,]:%99[^:,]%n", cert, ca, &n) == 2
...
static struct ns_connection *accept_conn(struct ns_connection *ls)
{
	...
	else if ((c = ns_add_sock(ls->mgr, sock, ls->callback, ls->user_data))
			== NULL)
	{
		closesocket(sock);
#ifdef NS_ENABLE_SSL
...
time_t ns_mgr_poll(struct ns_mgr *mgr, int milli)
{
	...
	else
	{
		...
		if (mgr->ctl[1] != INVALID_SOCKET && FD_ISSET(mgr->ctl[1], &read_set))
		{
			...
			int len = (int) recv(mgr->ctl[1], (char *) &ctl_msg,
					sizeof(ctl_msg), 0);
			send(mgr->ctl[1], ctl_msg.message, 1, 0);
			if (len
					>= (int) sizeof(ctl_msg.callback)&& ctl_msg.callback != NULL)
			{
...
#ifdef NS_ENABLE_SSL
	{	static int init_done; if (!init_done)
...
void ns_mgr_free(struct ns_mgr *s)
{
	...
	if (s->ctl[0] != INVALID_SOCKET)
		closesocket(s->ctl[0]);
	if (s->ctl[1] != INVALID_SOCKET)
		closesocket(s->ctl[1]);
	s->ctl[0] = s->ctl[1] = INVALID_SOCKET;
...
enum
{
	...
	CGI_INTERPRETER, CGI_PATTERN,
#endif
	DAV_AUTH_FILE, DAV_ROOT, DOCUMENT_ROOT,
...
#endif
#ifndef MONGOOSE_NO_FILESYSTEM
	HIDE_FILES_PATTERN, HEXDUMP_FILE, INDEX_FILES,
#endif
	LISTENING_PORT,
#ifndef _WIN32
	RUN_AS_USER,
...
#endif
#ifndef MONGOOSE_NO_CGI
		"cgi_interpreter", NULL, "cgi_pattern", DEFAULT_CGI_PATTERN,
#endif
		"dav_auth_file", NULL, "dav_root", NULL, "document_root", NULL,
#ifndef MONGOOSE_NO_DIRECTORY_LISTING
		"enable_directory_listing", "yes",
#endif
#endif
		"enable_proxy", NULL, "extra_mime_types", NULL,
#if !defined(MONGOOSE_NO_FILESYSTEM) && !defined(MONGOOSE_NO_AUTH)
		"global_auth_file", NULL,
#endif
#ifndef MONGOOSE_NO_FILESYSTEM
		"hide_files_patterns", NULL, "hexdump_file", NULL, "index_files",
		...
		...)
{
	...
	{
		va_start(ap, fmt);
		body_len += mg_vsnprintf(body + body_len, sizeof(body) - body_len, fmt,
				ap);
		va_end(ap);
	}
...
static process_id_t start_process(const char *interp, const char *cmd,
		const char *env, const char *envp[], const char *dir, sock_t sock)
{
	char buf[500];
...
static void open_cgi_endpoint(struct connection *conn, const char *prog)
{
	...
	if (start_process(conn->server->config_options[CGI_INTERPRETER], prog,
			blk.buf, blk.vars, dir, fds[1]) != 0)
	{
	...
	if (conn->ns_conn->flags & NSF_BUFFER_BUT_DONT_SEND)
	{
		...
		if (len < 0 || len > (int) sizeof(buf))
		{
			...
			send_http_error(conn, 500,
					"CGI program sent malformed headers: [%.*s]", len, io->buf);
		}
		else
		{
			memset(&c, 0, sizeof(c));
...
static int is_valid_http_method(const char *s)
{
	return !strcmp(s, "GET") || !strcmp(s, "POST") || !strcmp(s, "HEAD")
			|| !strcmp(s, "CONNECT") || !strcmp(s, "PUT")
			|| !strcmp(s, "DELETE") || !strcmp(s, "OPTIONS")
			|| !strcmp(s, "PROPFIND") || !strcmp(s, "MKCOL")
			|| !strcmp(s, "PATCH");
}
...
int mg_match_prefix(const char *pattern, ssize_t pattern_len, const char *str)
{
	...
	for (; i < pattern_len; i++, j++)
	{
		...
		else if (pattern[i] == '*')
		{
			...
			do
			{
				res = mg_match_prefix(pattern + i, pattern_len - i,
						str + j + len);
			} while (res == -1 && len-- > 0);
...
static int is_dav_request(const struct connection *conn)
{
	...
	return !strcmp(s, "PUT") || !strcmp(s, "DELETE") || !strcmp(s, "MKCOL")
			|| !strcmp(s, "PROPFIND");
}
		...
		size_t buf_len, file_stat_t *st)
{
	...
	const char *root =
			...
			is_dav_request(
					conn) && conn->server->config_options[DAV_ROOT] != NULL ?
...
static time_t parse_date_string(const char *datetime)
{
	...
	if (((sscanf(datetime, "%d/%3s/%d %d:%d:%d", &day, month_str, &year, &hour,
			...
			|| (sscanf(datetime, "%*3s, %d %3s %d %d:%d:%d", &day, month_str,
		...
		file_stat_t *st, const char *extra_headers)
{
			...
			&& r2 >= 0)
	{
		...
		conn->cl =
				n == 2 ?
						(r2 > conn->cl ? conn->cl : r2) - r1 + 1 :
						conn->cl - r1;
		mg_snprintf(range, sizeof(range), "Content-Range: bytes "
				"%" INT64_FMT "-%" INT64_FMT "/%" INT64_FMT "\r\n", r1,
				r1 + conn->cl - 1, (int64_t) st->st_size);
		msg = "Partial Content";
	...
	n = mg_snprintf(headers, sizeof(headers), "HTTP/1.1 %d %s\r\n"
			"Date: %s\r\n"
			"Last-Modified: %s\r\n"
			"Etag: %s\r\n"
			"Content-Type: %.*s\r\n"
	...
	"%s%s%s\r\n", conn->mg_conn.status_code, msg, date, lm, etag,
			...
			suggest_connection_header(&conn->mg_conn), range,
...
static void print_dir_entry(const struct dir_entry *de)
{
	...
	else
	{
		...
		{
			mg_snprintf(size, sizeof(size), "%.1fG",
	...
	mg_printf_data(&de->conn->mg_conn, "<tr><td><a href=\"%s%s\">%s%s</a></td>"
			"<td>&nbsp;%s</td><td>&nbsp;&nbsp;%s</td></tr>\n", href, slash,
			de->file_name, slash, mod, size);
}
...
static void send_directory_listing(struct connection *conn, const char *dir)
{
	...
	int i, num_entries, sort_direction =
			conn->mg_conn.query_string != NULL
	...
	mg_printf_data(&conn->mg_conn, "<html><head><title>Index of %s</title>"
			"<style>th {text-align: left;}</style></head>"
			"<body><h1>Index of %s</h1><pre><table cellpadding=\"0\">"
			"<tr><th><a href=\"?n%c\">Name</a></th>"
			"<th><a href=\"?d%c\">Modified</a></th>"
			"<th><a href=\"?s%c\">Size</a></th></tr>"
			"<tr><td colspan=\"3\"><hr></td></tr>", conn->mg_conn.uri,
			conn->mg_conn.uri, sort_direction, sort_direction, sort_direction);
		...
		file_stat_t *stp)
{
	...
	mg_printf(&conn->mg_conn, "<d:response>"
			"<d:href>%s</d:href>"
			"<d:propstat>"
			"<d:prop>"
		...
		file_stat_t *stp, int exists)
{
	...
	else
	{
		...
		if (S_ISDIR(stp->st_mode) && (depth == NULL || strcmp(depth, "0") != 0))
		{
			struct dir_entry *arr = NULL;
			...
			for (i = 0; i < num_entries; i++)
			{
				...
				mg_url_encode(de->file_name, strlen(de->file_name), buf,
						sizeof(buf));
...
{
	file_stat_t st;
	const char *range, *cl_hdr = mg_get_header(&conn->mg_conn,
			"Content-Length");
	int64_t r1, r2;
	int rc;
	...
	else if ((conn->endpoint.fd = open(path,
			O_RDWR | O_CREAT | O_TRUNC | O_BINARY, 0644)) < 0)
	{
	...
	{
		DBG(
				("PUT [%s] %lu", path, (unsigned long) conn->ns_conn->recv_iobuf.len));
		...
		int is_directory)
{
	...
	{
		mg_snprintf(name, sizeof(name), "%s%c%s", path, '/',
				PASSWORDS_FILE_NAME);
		fp = fopen(name, "r");
	}
	else
...
int mg_authorize_digest(struct mg_connection *c, FILE *fp)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL)
	{
		if (sscanf(line, "%[^:]:%[^:]:%s", f_user, f_domain, ha1) == 3
			...
			return check_password(c->request_method, ha1, uri, nonce, nc,
					cnonce, qop, resp);
	}
	return MG_FALSE;
}
		...
		char *tag, int include_level)
{
	...
	if (sscanf(tag, " virtual=\"%[^\"]\"", file_name) == 1)
	{
		...
		mg_snprintf(path, sizeof(path), "%s%c%s", opts[DOCUMENT_ROOT], '/',
				file_name);
	}
	...
	{
		mg_printf(conn, "Cannot open SSI #include: [%s]: fopen(%s): %s", tag,
				path, strerror(errno));
	}
	else
	{
	...
	while ((ch = fgetc(fp)) != EOF)
	{
		if (in_ssi_tag && ch == '>')
		{
			...
			else
			{
				...
				{
					mg_printf(conn, "%s: unknown SSI " "command: \"%s\"", path,
							buf);
				}
			}
			len = 0;
...
static void proxy_request(struct ns_connection *pc, struct mg_connection *c)
{
	...
	ns_printf(pc, "%s %s%s%s HTTP/%s\r\n", c->request_method, c->uri,
			c->query_string ? "?" : "", c->query_string ? c->query_string : "",
			c->http_version);
...
int mg_forward(struct mg_connection *c, const char *addr)
{
	...
	if ((pc = ns_connect(&conn->server->ns_mgr, addr, mg_ev_handler, conn))
			== NULL)
	{
...
static void proxify_connection(struct connection *conn)
{
	...
	if (sscanf(url, "%499[^: ]:%hu%n", host, &port, &n) != 2
			&& sscanf(url, "%9[a-z]://%499[^: ]:%hu%n", proto, host, &port, &n)
					!= 3
		...
		file_stat_t *st, int exists, const char *extra_headers)
{
	...
	else if (mg_match_prefix(cgi_pat, strlen(cgi_pat), path) > 0)
	{
		...
		send_http_error(conn, 501, NULL);
			...
			strlen(conn->server->config_options[SSI_PATTERN]), path) > 0)
	{
...
static void open_local_endpoint(struct connection *conn, int skip_user)
{
	...
	else if ((!is_dav_request(conn)
			&& !is_authorized(conn, path, exists && S_ISDIR(st.st_mode)))
...
{
	unsigned short n;
	return uri[0] == '/' || strcmp(uri, "*") == 0 || // OPTIONS method can use asterisk URI
...
static void on_recv_data(struct connection *conn)
{
	...
	{
		int res = call_user(conn, MG_HTTP_MSG);
		if (MG_TRUE == res)
		{
...
static void log_access(const struct connection *conn, const char *path)
{
	...
	mg_parse_header(mg_get_header(&conn->mg_conn, "Authorization"), "username",
			user, sizeof(user));
	fprintf(fp, "%s - %s [%s] \"%s %s%s%s HTTP/%s\" %d 0", c->remote_ip,
			user[0] == '\0' ? "-" : user, date,
			c->request_method ? c->request_method : "-", c->uri ? c->uri : "-",
			c->query_string ? "?" : "", c->query_string ? c->query_string : "",
...
static void close_local_endpoint(struct connection *conn)
{
	...
	int keep_alive =
			should_keep_alive(&conn->mg_conn)
					&& (conn->endpoint_type == EP_FILE
							|| conn->endpoint_type == EP_USER);
	DBG(
			("%p %d %d %d", conn, conn->endpoint_type, keep_alive, conn->ns_conn->flags));
...
static void transfer_file_data(struct connection *conn)
{
	...
	if (conn->ns_conn->send_iobuf.len > sizeof(buf) * 2)
		return;

	// Do not send anyt
	n = read(conn->endpoint.fd, buf,
			conn->cl < (int64_t) sizeof(buf) ?
...
int mg_get_var_n(const struct mg_connection *conn, const char *name, char *dst,
		size_t dst_len, int n)
{
	int len = get_var(conn->query_string,
			conn->query_string == NULL ? 0 : strlen(conn->query_string), name,
			dst, dst_len, n);
	if (len == -1)
	{
		...
		int *data_len)
{
	...
	for (n = bl; (ll = get_line_len(buf + n, hl - n)) > 0; n += ll)
	{
		...
		{
			parse_header(buf + n + cdl, ll - (cdl + 2), "name", var_name,
					var_name_len);
...
void mg_copy_listeners(struct mg_server *s, struct mg_server *to)
{
	...
	for (c = ns_next(&s->ns_mgr, NULL); c != NULL; c = ns_next(&s->ns_mgr, c))
	{
		...
		if ((c->flags & NSF_LISTENING) && (tmp =
				(struct ns_connection *) NS_MALLOC(sizeof(*tmp))) != NULL)
		{
...
static void on_accept(struct ns_connection *nc, union socket_address *sa)
{
	...
	if (!check_acl(server->config_options[ACCESS_CONTROL_LIST],
			ntohl(*(uint32_t *) &sa->sin.sin_addr)) || (conn =
			(struct connection *) NS_CALLOC(1, sizeof(*conn))) == NULL)
	{
...
static void mg_ev_handler(struct ns_connection *nc, int ev, void *p)
{
	...
	case NS_POLL:
		...
		// Expire idle connections
		{
			...
			if (nc->listener != NULL
					&& nc->last_io_time + MONGOOSE_IDLE_TIMEOUT_SECONDS
							< current_time)
			{
...
{
	struct mg_server *server = (struct mg_server *) NS_CALLOC(1,
			sizeof(*server));
